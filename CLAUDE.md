...
- MCP SDK v1.5.0 enables DNS rebinding protection by default (`DisableLocalhostProtection` is not set); the default bind address `127.0.0.1:8080` passes validation. SDK-level Content-Type verification is also enabled by default, rejecting POST requests without `application/json`. Tool input-validation errors are returned as tool results (not JSON-RPC errors) as of v1.5.0.
...
- `--github-client-secret` flag sets the GitHub OAuth app client secret. Falls back to `GITHUB_CLIENT_SECRET` env var if the flag is empty (avoids exposing secrets in process listings).
- `--allowed-users` flag sets comma-separated allowed GitHub usernames (empty = allow all authenticated users).
- `--jwt-secret` flag sets the HMAC-SHA256 signing key for JWT tokens (auto-generates a 32-byte random key if empty). Falls back to `JWT_SECRET` env var if the flag is empty.
...
- `src/app/auth/store.go` implements an in-memory `Store` with `sync.RWMutex` for auth codes (one-time use with expiry), refresh tokens (rotation via consume-and-reissue), and registered clients (dynamic registration per RFC 7591). All entity types are capped (auth codes: 10000, refresh tokens: 10000, clients: 1000) with expired-entry eviction when caps are reached. `SaveAuthCode` and `SaveRefreshToken` return errors when caps are exceeded. The store provides `ValidateAndConsumeAuthCode` (atomically retrieves, validates via caller-supplied function, and deletes under a single write lock), `GetAuthCode` (read-only copy without delete), `DeleteAuthCode` (delete by key), and `RotateRefreshToken` (atomically consumes old token and saves new token with identity field carry-over, restoring the old token if the cap is exceeded). The store runs a periodic cleanup goroutine (`StartCleanup`/`StopCleanup`) every 5 minutes to evict expired entities. Client registrations expire after 30 days (`clientTTL`). The cleanup lifecycle is managed via fx lifecycle hooks in `auth.Module`.
- `src/app/auth/jwt.go` implements `IssueAccessToken` (HMAC-SHA256 JWT with iss/sub/exp/nbf/iat/jti/scope claims), `IssueRefreshToken` (random 32-byte base64url), and `NewTokenVerifier` (returns `auth.TokenVerifier` that maps JWT to `auth.TokenInfo`).
...
- `src/app/clients/github/di.go` contains `github.Module` (`fx.Module("github")`), which provides `Client`. `StreamableModules` composes it as `ghclient.Module`.
...
- `src/app/auth/di.go` contains `auth.Module`, which provides: AllowedUsers, validated Issuer, JWTSecret, Store, AuthorizationServerMetadata, ProtectedResourceMetadata, TokenVerifier. `StreamableModules` composes `auth.Module`.
- `src/app/api/rest/handler.go` implements `Handler` with OAuth HTTP endpoints: `HandleAuthServerMetadata` (GET `/.well-known/oauth-authorization-server`), `HandleAuthorize` (GET `/oauth/authorize` - validates PKCE, HMAC-signs state, redirects to GitHub), `HandleCallback` (GET `/oauth/callback` - verifies state, exchanges GitHub code, checks allowlist, issues auth code; sanitizes GitHub error parameters against a known-codes allowlist), `HandleToken` (POST `/oauth/token` - authorization_code with atomic PKCE validate-and-consume via `ValidateAndConsumeAuthCode` and refresh_token grants), `HandleRegister` (POST `/oauth/register` - dynamic client registration). `NewHandler` returns `(*Handler, error)` because it derives a separate HKDF state-signing key from the JWT secret (using `golang.org/x/crypto/hkdf`). `HandlerParams` references types from `auth` and `clients/github`.
- `src/app/api/rest/di.go` contains `rest.Module` (`fx.Module("rest")`), which provides `Handler`. `StreamableModules` composes `rest.Module`.
- The API router (in streamable mode) registers OAuth discovery and flow endpoints (delegating to `rest.Handler`), applies a dedicated tight per-IP rate limiter (2 req/s, burst 5) to `POST /oauth/register`, and wraps `/mcp` with `auth.RequireBearerToken` middleware that validates JWT bearer tokens and sets `ResourceMetadataURL` to `/.well-known/oauth-protected-resource`.
...
- `Dockerfile` is a multi-stage build: `golang:1.25-alpine` builder compiles a static binary (`CGO_ENABLED=0`, `-trimpath`, `-s -w`, ldflags for `hjarta-di.Version`/`DIVersion`/`CompiledAt`) and the runtime is `FROM scratch` with CA certs copied from the builder. Entry point is `/intervals-icu-mcp`; default `CMD` runs streamable transport on `0.0.0.0:8080`. Build args: `VERSION`, `DI_VERSION` (default `0.5.0`), `COMPILED_AT`, `TARGETOS`, `TARGETARCH`.
- `.dockerignore` is an exclusion-based ignore file. Files not explicitly excluded are included in the build context. The effective inclusions are `src/`, `go.mod`, `go.sum`, and `README.md`. New root-level files should be added to `.dockerignore` unless needed at build time.
- `Makefile` provides local developer helpers: `make build` (native-arch, `--load`), `make push` (multi-arch `linux/amd64,linux/arm64`, pushes `<image>:<VERSION>` and `<image>:latest`), `make test`, `make lint`, `make tidy`. Variables: `IMAGE`, `VERSION`, `DI_VERSION`, `COMPILED_AT`, `PLATFORMS`.
- `.github/workflows/ci.yml` runs on `pull_request` and `push` to `main`: three parallel jobs (test, lint, multi-arch docker build without push). Uses pinned action SHAs. Go 1.25.
- `.github/workflows/release.yml` runs on `release: published` and `workflow_dispatch` (with optional `version` input). Resolves the tag, checks out at that ref, logs in to GHCR with `GITHUB_TOKEN`, uses `docker/metadata-action` to produce semver tags (`X.Y.Z`, `X.Y`, `latest`), and builds+pushes a multi-arch image to `ghcr.io/0xalexb/intervals-icu-mcp`.
- Release procedure: cut a GitHub Release off a `vX.Y.Z` tag; the workflow builds and publishes the image. `workflow_dispatch` with an explicit `version` input re-publishes an existing tag.
...
- Test files have exclusions for `err113`, `funlen`, `exhaustruct`, `gosec`, `cyclop`, `dupl`, `varnamelen`, `goconst`, `wsl_v5`, `lll`, `perfsprint`, `errorlint`, `errcheck`, `tagalign`, `forcetypeassert`, and `gochecknoglobals`.
...
- Auth dependencies: `github.com/golang-jwt/jwt/v5` (JWT issuance/verification), `github.com/gofrs/uuid/v5` (client ID generation), `golang.org/x/crypto` (HKDF key derivation for state signing)
