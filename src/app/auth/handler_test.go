...
const (
	testJWTSecret   = JWTSecret("test-handler-jwt-secret-key-1234")
	testIssuer      = Issuer("https://auth.example.com")
	testGHClientID  = GitHubClientID("gh-client-id")
	testGHSecret    = GitHubClientSecret("gh-client-secret")
)

func newTestHandler(ghTokenURL, ghUserURL string) *Handler {
	return &Handler{
		store:          NewStore(),
		allowedUsers:   AllowedUsers{"alice", "bob"},
		ghClientID:     testGHClientID,
		ghClientSecret: testGHSecret,
		jwtSecret:      testJWTSecret,
		issuer:         testIssuer,
		metadata:       NewAuthorizationServerMetadata(testIssuer),
...
func TestHandleToken_AuthorizationCodeGrant(t *testing.T) {
	...
	verifier := NewTokenVerifier(testJWTSecret, testIssuer)
